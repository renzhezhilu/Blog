...
!
function(t) {
	"function" == typeof define && define.amd ? define([], t) : "object" == typeof exports ? module.exports = t: window && (window.Syntaxy = t(window.jQuery || null))
} (function(t) {
	...
	return e.prototype = {
		...
		setTarget: function(t) {
			return "object" == typeof t && (this.target = t, this.setCode(t.innerHTML || ""), this.setOptions({
		...
		setOptions: function(t) {
			if ("object" == typeof t) for (var e in t) t.hasOwnProperty(e) && (this.options[e] = t[e]);
			return this
		},
		setCode: function(t) {
			return this.code = ("string" == typeof t ? t: "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&nbsp;|\u00a0/gi, "s").replace(/\t/g, "ssss").replace(/[\r]+/g, "").replace(/^[\n]+/, "").replace(/[\s\t\n\uFEFF\xA0]+$/, ""),
			this
		},
		processCode: function() {
			this.error = "";
			var t = "";
			if (this.code) if (this.options.codeType) if (this.hasFilter(this.options.codeType)) try {
				t = this.applyFilter(this.options.codeType, this.code).replace(/</g, "&lt;").replace(new RegExp(this.options.tagClose, "g"), "</" + this.options.tagName + ">").replace(new RegExp(this.options.tagOpen + "([A-Z]+)" + this.options.tagSplit, "g"),
				function(t, e) {
				...
				t = this.getCode(!0)
			} else this.error = "The syntax type specified (" + this.options.codeType + ") could not be found.",
			t = this.getCode(!0);
			else this.error = "The syntax type for this container has not been specified.",
			t = this.getCode(!0);
			else this.error = "This container does not have any code to be highlighted.",
			t = "// No code available.";
			return t
		},
		getCode: function(t) {
		...
		addFilter: function(t, e) {
			return t = this.toString(t, "").replace(/[^\w]+/g, ""),
			t && "function" == typeof e && (this.reglist[t] = e),
			this
		},
		...
		wrapClass: function(t, e, i, s) {
			if (e = this.toString(e, ""), i === !0 && (e = e.replace(new RegExp(this.options.tagOpen + "([A-Z]+)" + this.options.tagSplit, "g"), ""), e = e.replace(new RegExp(this.options.tagClose, "g"), "")), s === !0) {
				var r = [];
				return e.split("\n").forEach(function(e) {
					e = e.replace(/^([\s]*)(.*?)([\s]*)$/,
					function(e, i, s, n) {
		...
		toClass: function(t) {
			return t = this.toString(t, "").toLowerCase(),
			this.options.classPrefix + t
		},
		toTag: function(t, e) {
			return t = this.toString(t, "").toUpperCase(),
			e = this.toString(e, ""),
			this.options.tagOpen + t + this.options.tagSplit + e + this.options.tagClose
		},
		...
		toNumeric: function(t, e) {
			return t = /^(\-|\+){0,1}[0-9\.]+$/.test(t) ? t: 0,
			e = /^(\-|\+){0,1}[0-9\.]+$/.test(e) ? e: 0,
			t || e
		},
		toBoolean: function(t, e) {
			return e = "boolean" == typeof e ? e: !1,
			/^(1|on|true|active|enabled?|Y)$/i.test(t) ? !0 : e
		},
		lineOutdent: function(t, e) {
			return t = t || "",
			e = parseInt(e || 0),
			"" === t.trim() ? "&nbsp;": e > 0 ? t.replace(new RegExp("^\\s{" + e + "}"), "") : t
		},
		lineClass: function(t, e) {
			var i = t % 2 ? "lighter": "darker",
			s = -1 !== e.indexOf(String(t)) ? "flash": i;
			return this.options.classPrefix + s
		},
		toggleClass: function(t, e) {
			if ("object" == typeof t && "string" == typeof e) if (t.classList && t.classList.toggle) t.classList.toggle(e);
		...
		render: function() {
			if (this.target) {
				...
				if (this.target.style.margin = "0", this.target.style.padding = "0", this.target.style.border = "0", this.target.style["text-align"] = "left", this.options.isInline === !0) this.target.style.display = "inline-block",
				this.target.innerHTML = '<span class="' + e("wrap-inline") + '">' + t + "</span>";
				else {
					for (var i = t.search(/\S|$/) || 0, s = t.split("\n"), r = 0, n = s.length, a = this.options.debugLines.split(","), o = this.options.startLine, l = this.options.wordWrap ? "wordwrap": "nowrap", p = "", c = "", h = "", u = 0; n > u; u++) {
					...
					this.options.minHeight && (p += "min-height: " + this.options.minHeight + "; "),
					this.options.maxHeight && (p += "max-height: " + this.options.maxHeight + "; "),
					this.error && (c = "!!!"),
					this.target.style.display = "block",
					this.target.innerHTML = '<div class="' + e("wrap") + '"><div class="' + e("header") + " " + e("clear") + '"><div class="' + e("left") + '">' + this.options.codeTitle + '</div><div class="' + e("right") + '"><button class="' + e("error-btn") + " " + e("important") + '" type="button" title="Process error">' + c + '</button> <button class="' + e("toggle-btn") + '" type="button" title="Toggle line numbers">☀</button> <span>' + r + " " + (1 === r ? "line": "行") + '</span></div></div><div class="' + e("scroller") + '" style="' + p + '">' + h + "</div></div>",
					this.error && this.target.querySelector("." + e("error-btn")).addEventListener("click",
					function(t) {
						alert(this.error)
					}.bind(this)),
					this.target.querySelector("." + e("toggle-btn")).addEventListener("click",
					function(t) {
		...
		factory: function() {
			this.reglist = {
				doctypes: function(t) {
					return t = t.replace(/(<!DOCTYPE[\w\W]+?>)/g, this.wrapClass("doctype", "$1")),
				...
				constants: function(t) {
					return t = t.replace(/(?=.*[A-Z])(\$?(\b(?![\d\#\'\"]+)([A-Z0-9\_]{2,})\b))(?![\:\'\"\(])/g, this.wrapClass("global", "$1")),
					t = t.replace(/(?=.*[\w]+)([\_]{2}[\w]+)/g, this.wrapClass("global", "$1"))
				},
				classes: function(t) {
					return t = t.replace(/(?=.*[a-z])\b(([A-Z]+[a-z0-9]+)+)/g, this.wrapClass("class", "$1"))
				},
				functions: function(t) {
					return t = t.replace(/(^|\b[\w\-\!\*]+)(\s*\()/g, this.wrapClass("function", "$1") + "$2")
				},
				numbers: function(t) {
					return t = t.replace(/([^\w\-]+)([+-]?(\.?\d)+)(?![\w\'\"])/g, "$1" + this.wrapClass("numeric", "$2")),
				...
				keywords: function(t) {
					...
					return i.split(",").forEach(function(i) {
						...
						t = t.replace(new RegExp("(^|[^\\.|\\>|\\-|\\/])([\\$|\\#]?\\b(" + i + ")\\b)(?![\\=|\\(|\\:])", "g"),
						function(t, i, s) {
				...
				keys: function(t) {
					return t = t.replace(/([\w\-]+)(([\s]+)?\:)(?=[\s]+)/g, this.wrapClass("key", "$1") + "$2")
				},
				tags: function(t) {
					return t.replace(/(<[^\!\?\%\#\=]\/?[^>]*>)/gi,
					function(t, e) {
						var i = "tag";
						return i = /^<\/?(style|link|script)/i.test(e) ? "hook": i,
				...
				markup: function(t) {
					return t = t.replace(/(<style[^>]*>)([^<]*)(<\/style>)/g,
					function(t, e, i, s) {
						return e + this.applyFilter("style", i) + s
					}.bind(this)),
					t = t.replace(/(?!.*src\=)(<script[^>]*>)([^<]*)(<\/script>)/g,
					function(t, e, i, s) {
						return e + this.applyFilter("default", i) + s
					}.bind(this)),
					t = t.replace(/(<\?php|<\?=?|<\%\=?)([\w\W]+?)(\?>|\%>)/g,
					function(t, e, i, s) {
						return e + this.applyFilter("default", i) + s
					}.bind(this)),
					t = t.replace(/(<!--[\s\S]*?-->)/g,
				...
				shell: function(t) {
					return t = this.applyFilter("constants", t),
					t = this.applyFilter("doctypes", t),
					t = this.applyFilter("functions", t),
				...
				json: function(t) {
					return t = this.applyFilter("strings", t),
					t = this.applyFilter("comments", t)
				},
				sql: function(t) {
					return t = this.applyFilter("constants", t),
